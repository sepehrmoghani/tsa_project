...
class PdfToExcelView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        try:
            the_pdf = request.POST.get("pdf_file")
            start_page = int(request.POST.get("start_page"))
            multiple_page = True if request.POST.get("multiple_page") == "true" else False

            dataframes = self.read_pdf(the_pdf + ".pdf", start_page, multiple_page)

            if dataframes:
                all_data = pd.concat(dataframes, ignore_index=True)
                excel_file = the_pdf + ".xlsx"
                all_data.to_excel(excel_file, index=False)
                response = HttpResponse(content_type='application/vnd.ms-excel')
                response['Content-Disposition'] = 'attachment; filename=' + excel_file
                all_data.to_excel(response, index=False)
                return response
            else:
